...
Monkey Island 1 SE (raw DXT, 1024-pixel grid, x then y in filenames):
...
Monkey Island 2 SE (gzip-compressed DXT, 1022-pixel stride with 2-pixel
overlap, y then x in filenames):

    layer0_chunk_0_0.dxt        -> image "layer0" at (0, 0)
    layer0_chunk_0_1022.dxt     -> image "layer0" at (1022, 0)
    layer0_chunk_0_2044.dxt     -> image "layer0" at (2044, 0)
...
followed by raw block data (MI1) or gzip-compressed block data (MI2):
...
def is_gzip_dxt(path: Path) -> bool:
    """Return True when the file stores gzip-compressed DXT payload (MI2 SE)."""
    with path.open("rb") as handle:
        header = handle.read(HEADER_SIZE + 2)
    return len(header) >= HEADER_SIZE + 2 and header[HEADER_SIZE : HEADER_SIZE + 2] == GZIP_MAGIC


def read_dxt_payload(data: bytes) -> bytes:
    """Return raw DXT block bytes after the 12-byte header."""
...
def parse_chunk_filename(
    path: Path, *, yx_coords: bool = False
    ...
    first = int(match.group("x"))
    second = int(match.group("y"))
    if yx_coords:
        return match.group("name"), second, first
    return match.group("name"), first, second
...
def crop_mi2_tile(
    ...
    """Trim MI2 overlap so adjacent 1024-wide tiles stitch on a 1022 stride."""
    _, width = rgba.shape[:2]
    has_left = any(px < x and py == y for px, py in positions)
    has_right = any(px > x and py == y for px, py in positions)

    x0 = MI2_TILE_OVERLAP if has_left else 0
    if has_right and not has_left:
        x1 = MI2_TILE_STRIDE
    elif has_right:
        x1 = min(x0 + MI2_TILE_STRIDE, width)
    else:
        x1 = width
    return rgba[:, x0:x1]
...
def merge_chunks(
    chunks: list[tuple[int, int, Path]], *, mi2_mode: bool = False
) -> Image.Image:
    """Merge (x, y, path) tiles into one image sized to their bounding box."""
    positions = {(x, y) for x, y, _ in chunks}
    ...
    for x, y, path in sorted(chunks, key=lambda t: (t[1], t[0])):
        ...
        if mi2_mode:
            rgba = crop_mi2_tile(rgba, x, y, positions)
            width = rgba.shape[1]
...
def directory_uses_mi2_format(directory: Path) -> bool:
    """Detect MI2-style gzip DXT tiles in a directory."""
    for path in directory.glob("*.dxt"):
        if is_gzip_dxt(path):
            return True
    return False
...
def group_chunks(
    directory: Path, *, yx_coords: bool = False
    ...
    for path in sorted(directory.glob("*.dxt")):
        parsed = parse_chunk_filename(path, yx_coords=yx_coords)
        if parsed is None:
            continue
        name, x, y = parsed
...
def list_standalone_dxts(directory: Path, *, yx_coords: bool = False) -> list[Path]:
    """Return .dxt files that are not chunk tiles (decode as whole images)."""
    return [
        path
        for path in sorted(directory.glob("*.dxt"))
        if parse_chunk_filename(path, yx_coords=yx_coords) is None
    ]


def find_dxt_directories(root: Path) -> list[tuple[Path, bool]]:
    """Return root/subdirs with .dxt files and whether they use MI2 gzip tiles."""
    ...
    for directory in candidates:
        ...
        if resolved in seen:
            continue
        seen.add(resolved)
        mi2_mode = directory_uses_mi2_format(directory)
        if group_chunks(directory, yx_coords=mi2_mode) or list_standalone_dxts(
            directory, yx_coords=mi2_mode
        ):
            found.append((directory, mi2_mode))
    return found
...
def process_dxt_directory(
    directory: Path, out_dir: Path, *, mi2_mode: bool = False
) -> list[Path]:
    """Merge chunk groups and decode standalone .dxt files into out_dir."""
    groups = group_chunks(directory, yx_coords=mi2_mode)
    standalone = list_standalone_dxts(directory, yx_coords=mi2_mode)
    if not groups and not standalone:
        raise FileNotFoundError(f"No .dxt files found in {directory}")

    format_label = "MI2 gzip/1022" if mi2_mode else "MI1 raw/1024"
    print(
        f"Found {len(groups)} chunk image(s) and {len(standalone)} standalone "
        f".dxt file(s) in {directory} [{format_label}]"
    )
    ...
    for name, chunks in sorted(groups.items()):
        ...
        image = merge_chunks(chunks, mi2_mode=mi2_mode)
...
def main() -> None:
    ...
    written: list[Path] = []
    try:
        for dxt_dir, mi2_mode in dxt_dirs:
            written.extend(process_dxt_directory(dxt_dir, out_dir, mi2_mode=mi2_mode))
